package com.fathur.ecommerce.controller;
...
import com.fathur.ecommerce.service.CartService;
import com.fathur.ecommerce.service.CategoryService;
import com.fathur.ecommerce.service.ProductService;
import com.fathur.ecommerce.service.UserService;
import com.fathur.ecommerce.utils.CommonUtils;

import jakarta.mail.MessagingException;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpSession;

import com.fathur.ecommerce.exception.EmailAlreadyRegisteredException;
import com.fathur.ecommerce.upload.UploadService;
import com.fathur.ecommerce.upload.UploadServiceImpl;
...
public class HomeViewController {
	...
	// by default call this method when any request come to this controller because
	// of @ModelAttribut
	@ModelAttribute
	public void getUserDetails(Principal principal, Model model) {
		if (principal != null) {
			...
			// System.out.println("Current Logged In User is :: HOME Controller ::
	...
	public String saveUserDetails(@ModelAttribute User user, @RequestParam("file") MultipartFile file, Model model,
		...
		if (!ObjectUtils.isEmpty(saveUser)) {
			if (!file.isEmpty()) {
				// full-path
				uploadService.storeFile(file, "profile_img");
			}
		...
		if (!uploadService.isImageFile(file)) {
			session.setAttribute("errorMsg", "Only JPEG and PNG image uploads are supported.");
	...
	public String forgetPasswordProcessing(@RequestParam String email, HttpSession session, HttpServletRequest request)
		...
		if (!ObjectUtils.isEmpty(user)) {
			...
			// URL Like This :
			// http://localhost:8080/reset-password?token=dfjdlkfjsldfdlfkdflkdfjdlk
			...
			if (isEmailSendToUser == true) {
				session.setAttribute("successMsg",
						"Please check your email, Password Reset Link has been sent to your email.");
			} else {
	...
	// public String resetPasswordOperation(@RequestParam String token,
	...
	// User updatedUser =
	// userService.updateUserWhileResetingPassword(userByToken);//this method only
	// update user's password and ResetTokens.
	...
	@PostMapping("/reset-password")
	public String resetPasswordOperation(@RequestParam String token, @RequestParam String password, HttpSession session,
			Model model) {
		...
		if (ObjectUtils.isEmpty(userByToken)) {
			...
			userByToken.setPassword(passwordEncoder.encode(password));
			userByToken.setResetTokens(null);
			User updatedUser = userService.updateUserWhileResetingPassword(userByToken);// this method only update
																						// user's password and
																						// ResetTokens.
			session.setAttribute("successMsg", "Password Changed Successfully");
			model.addAttribute("msg", "Password Changed Successfully");
